...
export function ProductCard({ product, categorySlug }: ProductCardProps) {
  ...
  return (
    <Link
      href={href}
      aria-label={
        soldOut ? `${product.name} (sold out)` : product.name
      }
      ...
      <div className="relative aspect-square w-full overflow-hidden bg-blush-100">
        {image ? (
          <Image
            ...
            className={
              soldOut
                ? "object-cover grayscale transition-transform group-hover:scale-105"
        ...
        {soldOut ? (
          <span className="absolute left-3 top-3 inline-flex items-center rounded-full bg-ink-900/90 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.22em] text-sugar-100 backdrop-blur">
            Sold out
          </span>
        ) : null}
      </div>
